        ...
        .map(function(pKey) {
            return {
                ...
                deplist: pDepencyTreeObject[pKey]
                            ...
                            // file exists. This prevents erroneous files from
                            // entering the dependency tree, but also from core node
                            // modules(path, fs, http, ...) from being mentioned
...
function getDeps(pDirOrFile, pExclude, pFormat, pFlatDefine) {
    ...
    if (fs.statSync(pDirOrFile).isFile()) {
        return !!pFlatDefine ?
                deps2FlatString(
                    extractFileDeps(lDeps, pDirOrFile), pFlatDefine, pDirOrFile
                ) :
